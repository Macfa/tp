...
try
{
	...
	if($totalPoint !== ($_POST['resultPoint']+$_POST['resultCash']))
		throw new Exception('사용할 포인트와 결제할 현금의 합이 총 결제금액과 다릅니다.', 3);	
...
switch ($use_pay_method) {
	case "100000000000":
		$orPaymentMethod = "신용카드";
		break;
	case "010000000000":
		$orPaymentMethod = "계좌이체";
		break;
	case "001000000000":
		$orPaymentMethod = "가상계좌";
		break;
}
...
DB::insert('tmOrder', array(
	...
	'orPoint' => $pointAmount,
	'orCash' => $cashAmount,
	'orPaymentMethod' => $orPaymentMethod,
	'orShipping' => $shipping,
