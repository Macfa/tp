	...
	for ($i=10; $i <= $Cellend; $i++) { 
		...
		for ($j = 'H'; $j != 'AA'; $j++) { 
			...
			if(isContain('65', $planCategory) === true ) {
				// $planCategory = '0,1,2';
				$planCategory1 = $arrPlanCategoryKt['kt'];
...
/*			foreach($planCategory1 as $plan) {
				foreach($plan as $value) {
					...
					$arr[$dvKey][$discountType][$planCategory][$applyType] = array(		// #1 이 부분은 foreach #1 과 호환. 엑셀형식으로 진행이 되어 데이터를 비교하기 위함
						'dvKey' => $dvKey,
						'rpDiscountType' => $discountType,
						'rpPlan' => $planCategory,
						'rpApplyType' => $applyType,
...
// echo "</pre>";
		...
		foreach ($arr as $dvKey => $discountType) {			// #1 , $arr #1 과 호환
			echo "<br/>";
			echo $dvKey.'&#9;';
